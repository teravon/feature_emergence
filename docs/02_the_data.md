...
**Where the key leaks.** The Signal-to-Noise Ratio (SNR) marks the time
samples whose power depends on the target intermediate value. Tall, narrow
peaks mean concentrated leakage; masking pushes the peaks down, because the
masks randomize the intermediate values.
