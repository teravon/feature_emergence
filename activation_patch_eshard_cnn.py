from profiling_and_attack import mlp_eshard, information, cnn
...
N_COMP = 40 if layer == 0 else 16 if layer ==1 else 8
...
for i in range(1, 101, 1):
    ...
    #snr_fast(dataset.x_profiling, dataset.share1_profiling[target_byte])
    ...
    if layer >= 0:
        ...
        snrlabel = snr_fast(s1_prof, dataset.profiling_labels)
    ...
    pi_og.append(information(new_model_out.predict(tmp_att.reshape((tmp_att.shape[0], og_1, og_2)) if layers[layer].__contains__("conv") else tmp_att), dataset.attack_labels, 9))

    for j in indices_share_1:
        s1_att[:, j] = np.average(s1_prof[indices_0_share_1, j])
    
    pathced_labs = hw(dataset.share2_attack[target_byte].astype(np.uint8))
    if layer >= 0:
    ...
    pi_patch_new.append( information(preds, pathced_labs,9))
    #information(preds, (dataset.share1_attack[target_byte].astype(np.uint8)),256)
...
np.savez(f"data/pi_eshard_cnn_patch_layer_{layer}_s1_no_ablate.npz",pi_og = np.array(pi_og),pi_patch_new = np.array(pi_patch_new), pi_patch_og = np.array(pi_patch_og))